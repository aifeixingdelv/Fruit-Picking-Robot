...
void amclposeCallback(const geometry_msgs::PoseWithCovarianceStamped :: ConstPtr & msg1)
{
	...
	if((abs(amclodom_px-amcl_px)>1)||(abs(amclodom_py-amcl_py)>1)||(abs(amclodom_Yaw-amcl_Yaw)>1.5))  //
	amcl_New = 2;
...
static void Getodom()
{
	...
	if(amcl_New == 1)//amcl_New == 1
	{
		...
		if((abs(amcl_ex)<0.2)&&(abs(amcl_ey)<0.2)&&(abs(amcl_eYa)<0.1))//amcl_pose修正
		{
			//amclodom_px = amcl_px;
			//amclodom_py = amcl_py;
			//amclodom_Yaw = amcl_Yaw;
   ...
   amcl_count++;
   if(s_both_wheel_stop == 0 )amclodom_count=15;
    if((amcl_count>15)||s_both_wheel_stop)
     {
		amcl_kx = 0.0;
		amcl_ky = 0.0;	
		amcl_kYa = 0.0;
...
*/
   ////ROS_INFO("Vx=%f,Vy= %f\n",vx,vy);
	...
	if(((amclodom_count>3000)&&s_both_wheel_stop)||(amcl_New == 2))
	{
...
int main(int argc, char** argv)
{
	...
	while(ros::ok())
	{
		...
		for(int i = 0; i < 6; i++)
			...
			odom.pose.covariance[4] = ang_yaw_;
